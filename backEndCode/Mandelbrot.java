...
public class Mandelbrot implements fractals {
	...
	private double constGapX = 2.75;
	private double constGapY = 2.6;
	private double constStartX =-2.15;
	private double constStartY =-1.3;
	private  double gapX =2.75/2047;
	private  double gapY = 2.6/2047;
	private double startX =-2.15;
	private double startY = -1.3;
	public Mandelbrot() {     //constructor of the Mandelbrot fractal
		_frac = new double[2048][2048];
		 _resultsTemp = new int[2048][2048];
	}
	
	@Override
	public double TransX(int x) {   //see interface comments
		double div = 2.75/2047;
		return -2.15+div*x;
	}

	@Override
	public double TransY(int y) {   //see interface comments
		double div = 2.6/2047;
		return -1.3+div*y;
	}
	...
	public int calcEscape(double x, double y) {  //see interface comments and for comment in Burning for details
		...
		while (distance <= _escapeDistance && passes < _escapeTimeUpperBound) { 
			...
			_yPrime = ClcY(x, y);
			//System.out.println(_xPrime);
			//System.out.println(_yPrime);
			passes = passes + 1;
